# bitmoji-lab companion

A drop-in MakeCode extension that pairs any BBC micro:bit V2 with the
[bitmoji-lab browser app](https://abourdim.github.io/bitmoji-lab/) —
...
2. **Advanced → Extensions → search** `abourdim/bitmoji-lab-companion`
   (or paste the repo URL directly)
3. Drag the `start bitmoji-lab companion` block into **on start**
...
That's it. Sensor stream starts, LEDs / paintbrushes / buzzer all respond.

## Blocks

| Block | What it does |
|---|---|
| `start bitmoji-lab companion` | Opens USB Web Serial service, shows a ● on LEDs when advertising, ✓ when connected. |
...
declares it, `main.ts` is the example, `bitmoji-lab.ts` is the block
