/**
 * Bitmoji Lab — Etsy Package Builder
 ...
 *
 * Produces:
 *   etsy-package/BitmojiLab-v1.0.0/          (folder)
 *   etsy-package/BitmojiLab-v1.0.0.zip       (final ZIP)
...
const ZIP_DIR = resolve(PKG, `BitmojiLab-${VERSION}`);
...
async function main() {
  console.log(`\n🎮 Bitmoji Lab Etsy Package Builder — ${VERSION}\n`);
  ...
  const zipPath = join(PKG, `BitmojiLab-${VERSION}.zip`);
  if (existsSync(zipPath)) rmSync(zipPath);
  let zipped = false;
  try { execSync(`cd "${PKG}" && zip -r "${zipPath}" "BitmojiLab-${VERSION}/"`, { stdio: 'pipe' }); zipped = true; } catch {}
  if (!zipped) try { execSync(`cd "${PKG}" && ditto -c -k --keepParent "BitmojiLab-${VERSION}" "${zipPath}"`, { stdio: 'pipe' }); zipped = true; } catch {}
