...
the **Bitmoji Lab** listing on Etsy.
...
├── BitmojiLab-v*/            📦 Build staging dir (rebuilt on demand)
├── BitmojiLab-v*.zip         📦 Final ZIP (this is what the buyer downloads)
│
...
- **What:** Node script that renders the 6 printable HTML templates to
  ...
  everything into `BitmojiLab-v<version>.zip`.
...
- **Outputs:**
  ...
  - `etsy-package/BitmojiLab-v<version>/` (staging dir, rebuilt)
  - `etsy-package/BitmojiLab-v<version>.zip` (the buyer ZIP)
...
- **What:** "We Make Emoji Light Shows!" poster in 5 big kid-safe steps.
...
11. Attach the digital file: `BitmojiLab-v<version>.zip`.
...
17. Keep old ZIPs tracked in Git (the `.gitignore` exemption keeps
    `etsy-package/BitmojiLab-v*.zip` in history) so you can always
